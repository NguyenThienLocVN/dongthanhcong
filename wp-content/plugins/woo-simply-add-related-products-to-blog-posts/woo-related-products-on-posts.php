...
// Checks to make sure this is a regular post and nothing else
	if ( is_single() && in_category($userSelectedCategory))  {
		...
		foreach ($wrpp_relateddProds as $wrpp_relateddProd) {
			...
			$wrpp_price = number_format($_product->get_price());
...
// Check if the custom field has a value.
			if ( !empty( $wrpp_key_value ) ) {
				...
				<div class="prodImg"><a href="' . $wrpp_link . '">' . $wrpp_img . '</a></div><div class="prodTitt"><p class="prodTitth4"><a href="' . $wrpp_link . '">' . $wrpp_tit . '</a></p><p class="prodPricce">' . $wrpp_price . '₫</p></div></div>';
			}
		}
...
// First check if there are Products and then check if there are Related Products for this blog post
		if ($wrpp_productCountzz > 0) {
			if ($wrpp_countter > 0) {
				...
				if ($wrpp_countter > 0) {
					$wrpp_content = $wrpp_content . '<div class="relprodscontainer">' . $wrpp_contenta . '</div>' . $wrpp_styles;}
					return $wrpp_content;
				}
...
// Add the action that displays the related products after the content
        add_shortcode('show_related_product_on_post', 'wrpp_displayProducts');
